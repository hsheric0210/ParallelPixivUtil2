namespace ParallelPixivUtil2
{
    ...
    public class PostprocessorSection
    {
        ...
        public string Parameters { get; set; } = "-s u {memberID} -x --pipe={ipcAddress} --db=\"{databasePath}\\{memberID}.p{fileIndex}.db\" -l \"{logPath}\\Postprocessor.{memberID}.p{fileIndex}.log\"";

        public bool ShowWindow { get; set; } = false;

        public bool PerPage { get; set; } = true;
    }
    ...
    public class SecureLookupSection
    {
        ...
        public string ArchiveCommand { get; set; } = "Add -Name={UserID} -Path=\"{Source}\" -Repo=\"{ArchiveFolder}\" -Tags=\"Username={UserName};Usertoken={UserToken};UserID={UserID}\" -Urls=\"{UserUrl}\" -ReusePreviousName -Append=\"{WorkingFolder}\\ArchiverQueue\"";
